module Util
  ...
  def self.manifest_diff(old, new)
    ...
    new_as.each do |ann|
      ...
      text_match = a["text"].present? && a["text"] == ann["text"]
      time_match = a["createdTimestamp"].present? && a["createdTimestamp"] == ann["createdTimestamp"]

      moved_i = old_as.index { |a| a["time"] == ann["time"] && (text_match || time_match) }
      if moved_i
